...
class GLDPC:
    ...
    def decode_cpp(self, llr_in, sigma2, max_iter, use_normalization = True, fix_iter = False):
        def bcjr_decode(llr):
            return self.CC_DECODER.decode(llr, sigma2, use_normalization)
        if fix_iter:
            return gldpc_decoder_fix_iter.decode_gldpc(
                bcjr_decode,
                self.H_GLDPC,
                self.H_LDPC,
