...
class GLDPC:
    ...
    def create_gldpc_matrix(self, H_LDPC, H_component):
        ...
        for i in range(m_ldpc):                                                                  
            ...
            selected_columns = np.random.choice(n_comp, size=n_comp, replace=False)
            repeated_columns = np.random.choice(num_ones-n_comp, size=num_ones-n_comp, replace=False)
            result_columns = np.concatenate([selected_columns, repeated_columns])
            np.random.shuffle(result_columns)
            # print('result_columns on iter: ', i, '\n', result_columns)
            ...
            for k, j in enumerate(ones_indices):
                ...
                H_gldpc[i * m_comp:(i + 1) * m_comp, j] = H_component[:, result_columns[k]]
    ...
    def create_row_layer_match(self):
        def count_initial_duplicates(arr):
            count = 0
            n = len(arr)
            cur = arr[0]
            wasIncr=False
            for i in range(1, n):
                if arr[i] == cur and not wasIncr:
                    count += 1
                    wasIncr=True
                else:
                    cur = arr[i]
        ...
        for i in range(self.H_LDPC.shape[0]):
            ...
            sorted_mapped_indexes, sorted_original_indexes1 = zip(*mapped_pairs) if mapped_pairs else ([], [])

            count_duplicates = count_initial_duplicates(sorted_mapped_indexes)

            sorted_original_indexes2=[]
            sorted_original_indexes1 = list(sorted_original_indexes1)
            for x in range(count_duplicates - 1, -1, -1):
                element = sorted_original_indexes1.pop(2 * x)
                sorted_original_indexes2.append(element)

            sorted_original_indexes2.reverse()
            row_layer_match[i] = {
                "layer": layer,
                "column_mapping": column_mapping,
                "sorted_original_indexes1": np.array(sorted_original_indexes1),  # Индексы в layer
                "sorted_original_indexes2": np.array(sorted_original_indexes2),
            ...
            # print('count_duplicates', count_duplicates)
            # print('sorted_original_indexes1:', sorted_original_indexes1)
            # print('sorted_original_indexes2:', sorted_original_indexes2)
    ...
    def decode(self, L, sigma2, maxIter):
        ...
        m_comp, n_comp = self.H_comp.shape
        ...
        for iter in range(maxIter):
            ...
            for i in range(m_ldpc):
                # print(self.row_layer_match[i])
                sorted_indexes1 = self.row_layer_match[i]['sorted_original_indexes1']
                sorted_indexes2 = self.row_layer_match[i]['sorted_original_indexes2']

                # print('sorted_indexes1',sorted_indexes1)

                # print('sorted_indexes2',sorted_indexes2)


                # Берем соответствующие значения из H_q
                llr_in_layer_decoder1 = H_q[i, sorted_indexes1]
                llr_in_layer_decoder2 = H_q[i, sorted_indexes2]
                # llr_in_layer_decoder2 = llr_in_layer_decoder2 + [0] * (n_comp - len(llr_in_layer_decoder2))
                llr_in_layer_decoder2 = np.concatenate((llr_in_layer_decoder2[:n_comp], np.zeros(n_comp - len(llr_in_layer_decoder2), dtype=int)))
                # print("llr_in_layer_decoder:", llr_in_layer_decoder)

                # Декодируем с помощью BCJR
                llr_from_layer_decoder1 = self.CC_DECODER.decode(llr_in_layer_decoder1, sigma2)
                llr_from_layer_decoder2 = self.CC_DECODER.decode(llr_in_layer_decoder2, sigma2)

                # print('llr by BCJR decoder:\n', llr_from_layer_decoder)

                # Записываем обратно в H_gamma по **изначальному** индексу
                # H_gamma[i, sorted_indexes] = llr_from_layer_decoder
                H_gamma[i, sorted_indexes1] = llr_from_layer_decoder1 - llr_in_layer_decoder1 # сделаем как бы экстринсик LLR
                # print('llr_in_layer_decoder2', llr_in_layer_decoder2)
                # print('llr_from_layer_decoder2', llr_from_layer_decoder2)

                H_gamma[i, sorted_indexes2] = (llr_from_layer_decoder2 - llr_in_layer_decoder2)[:len(sorted_indexes2)] # сделаем как бы экстринсик LLR
